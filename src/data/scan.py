...
def get_PSF(r_max=None, res_ratio=(1,1,3), threshold=1e-3, device=torch.device("cpu")):
    ...
    grid_z, grid_y, grid_x = torch.meshgrid(x, x, x)
    psf = torch.exp(-0.5 * (grid_x**2/sigma_x**2 + grid_y**2/sigma_y**2 + grid_z**2/sigma_z**2))
...
def init_stack_transforms(n_slice, gap, restricted, txy, device):
    ...
    tz = (torch.arange(0, n_slice, device=device, dtype=torch.float32) - (n_slice-1)/2.) * gap
    if txy:
...
class Scanner:
    ...
    def get_resolution(self, data):
        ...
        if hasattr(self.resolution_slice_fac, "__len__"):
            resolution_slice = np.random.uniform(self.resolution_slice_fac[0] * resolution, 
                min(self.resolution_slice_fac[-1] * resolution, self.resolution_slice_max))
        else:
        ...
        if hasattr(self.slice_thickness, "__len__"):
            data['slice_thickness'] = np.random.uniform(self.slice_thickness[0], self.slice_thickness[-1])
        else:
    ...
    def scan(self, data):
        ...
        if res_r != res:
            ...
            grid =  torch.stack(torch.meshgrid(*grids)[::-1], -1).unsqueeze_(0)
        ...
        psf_rec = get_PSF(res_ratio=(res_s/res_r, res_s/res_r, s_thick/res_r), device=device)
        ...
        if self.slice_size is None:
            ...
            ss = self.slice_size
        ns = int(max(vs) * res / s_thick) + 2

        down_factor = res_s * res_s * s_thick / res_r / res_r / res_r
        ...
        while True:
            ...
            transform_init = init_stack_transforms(ns, s_thick, self.restrict_transform, self.txy, device)
            ts = self.sample_time(ns)
            transform_motion = sample_motion(ts, device)
            # interleaved acquisition
            interleave_idx = interleave_index(ns, np.random.randint(2, int(np.sqrt(ns)) + 1))
            ...
            slices = slice_acquisition(mat, data['volume'], None, None, psf_acq, (ss, ss), res_s/res, False, False)
            slices_no_psf = slice_acquisition(mat, data['mask'], None, None, get_PSF(0, device=device), (ss, ss), res_s/res, False, False)
            # remove zeros
            ...
            positions.append(torch.arange(slices.shape[0], dtype=slices.dtype, device=device) - slices.shape[0]//2)

            if len(stacks) >= self.max_num_stack or (
                len(stacks) >= self.min_num_stack and (np.random.rand() > down_factor - len(stacks))
            ):
                break
        # add stack index
        positions = torch.cat([torch.stack((positions[i], torch.ones_like(positions[i]) * i), -1) for i in range(len(stacks))], 0)
