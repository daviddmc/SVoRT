...
class SRRtransformer(nn.Module):
    def __init__(self, n_res=34, n_layers=4, n_head=4, d_in=8, d_out=1, d_model=256, d_inner=512, dropout=0.1):
        super().__init__()
        self.srr = SRR(n_iter=2, use_CG=True)
        self.img_encoder = ResNet(n_res=n_res, d_model=d_model, pretrained=False, d_in=2)
        self.pos_emb = PositionalEncoding(d_model, d_in)
        self.encoder = TransformerEncoder(n_layers=n_layers, n_head=n_head, d_k=d_model//n_head, d_v=d_model//n_head, d_model=d_model, d_inner=d_inner, dropout=dropout)
        self.fc = nn.Linear(d_model, d_out)

    def forward(self, theta, transforms, slices, volume, params, idx):
        slices_est = slice_acquisition(transforms, volume, None, None, params['psf'], params['slice_shape'], params['res_s'] / params['res_r'], False, params['interp_psf'])
...
class SVRtransformer(nn.Module):
    def __init__(self, n_res=34, n_layers=4, n_head=4, d_in=8, d_out=6, d_model=256, d_inner=512, dropout=0.1, res_d_in=3, res_scale=1):
        super().__init__()
        if isinstance(n_res, nn.Module):
            self.img_encoder = n_res
        else:
            self.img_encoder = ResNet(n_res=n_res, d_model=d_model, pretrained=False, d_in=res_d_in)
        self.pos_emb = PositionalEncoding(d_model, d_in)
        self.encoder = TransformerEncoder(n_layers=n_layers, n_head=n_head, d_k=d_model//n_head, d_v=d_model//n_head, d_model=d_model, d_inner=d_inner, dropout=dropout)
        self.fc = nn.Linear(d_model, d_out)
    ...
    def pos_augment(self, slices, slices_est):
        ...
        y, x = torch.meshgrid(y, x) # hxw
        if slices_est is not None:       
            slices = torch.cat([slices, slices_est,
              y.view(1, 1, h, w).expand(n, -1, -1, -1), x.view(1, 1, h, w).expand(n, -1, -1, -1)], 1)
        else:
            if self.res_d_in == 3:
                slices = torch.cat([slices, 
                  y.view(1, 1, h, w).expand(n, -1, -1, -1), x.view(1, 1, h, w).expand(n, -1, -1, -1)], 1)
            else:
                slices = torch.cat([slices, 0*slices,
                  y.view(1, 1, h, w).expand(n, -1, -1, -1), x.view(1, 1, h, w).expand(n, -1, -1, -1)], 1)
        return slices
    ...
    def forward(self, theta, slices, pos, volume, params):
        ...
        if y is not None:
            with torch.no_grad():
                ...
                y = slice_acquisition(transforms, y, None, None, params['psf'], params['slice_shape'], params['res_s'] / params['res_r'], False, params['interp_psf'])
